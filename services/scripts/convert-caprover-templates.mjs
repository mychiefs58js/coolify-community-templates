...
const version =
  template.caproverOneClickApp.variables.find(
    (v) => v.id === "$$cap_APP_VERSION" || v.id === "$$cap_version"
  ).defaultValue || "latest";

newTemplate.name = template.caproverOneClickApp.displayName;
newTemplate.type = template.caproverOneClickApp.displayName
  .replaceAll(" ", "")
  .toLowerCase();
...
for (const service of Object.keys(template.services)) {
  ...
  const FROM = serviceTemplate.caproverExtra?.dockerfileLines?.find((line) =>
    line.startsWith("FROM")
  );
  if (serviceTemplate.image) {
    newService.image = serviceTemplate.image
      .replaceAll("cap_APP_VERSION", "core_version")
      .replaceAll("cap_version", "core_version");
  } else if (FROM) {
    newService.image = FROM.split(" ")[1]
      .replaceAll("cap_APP_VERSION", "core_version")
      .replaceAll("cap_version", "core_version");
  }

  const CMD = serviceTemplate.caproverExtra?.dockerfileLines?.find((line) =>
    line.startsWith("CMD")
  );
  if (serviceTemplate.command) {
    newService.command = serviceTemplate.command;
  } else if (CMD) {
    newService.command = CMD.replace("CMD ", "")
      .replaceAll('"', "")
      .replaceAll("[", "")
      .replaceAll("]", "")
      .replaceAll(",", " ")
  ...
  const ENTRYPOINT = serviceTemplate.caproverExtra?.dockerfileLines?.find(
    (line) => line.startsWith("ENTRYPOINT")
  );

  if (serviceTemplate.entrypoint) {
    newService.command = serviceTemplate.entrypoint;
  } else if (ENTRYPOINT) {
    newService.entrypoint = ENTRYPOINT.replace("ENTRYPOINT ", "")
      .replaceAll('"', "")
      .replaceAll("[", "")
      .replaceAll("]", "")
      .replaceAll(",", " ")
  ...
  if (
    serviceTemplate.environment &&
    Object.keys(serviceTemplate.environment).length > 0
  ) {
    for (const env of Object.keys(serviceTemplate.environment)) {
      const foundCaproverVariable = caproverVariables.find(
        (item) => item.id === serviceTemplate.environment[env]
      );

      let value = null;
      let defaultValue = foundCaproverVariable?.defaultValue
        ? foundCaproverVariable?.defaultValue
            .toString()
            ?.replace("$$cap_gen_random_hex", "$$$generate_hex")
        : "";

      if (
        serviceTemplate.environment[env].startsWith(
          "srv-captain--$$cap_appname"
        )
      ) {
        value = `$$config_${env}`.toLowerCase();
        defaultValue = serviceTemplate.environment[env]
          .replaceAll("srv-captain--$$cap_appname", "$$$id")
          .replace("$$cap_", "")
          .replaceAll("captain-overlay-network", `$$$config_${env}`)
          .toLowerCase();
      } else {
        value =
          "$$config_" +
          serviceTemplate.environment[env]
            .replaceAll("srv-captain--$$cap_appname", "$$$id")
            .replace("$$cap_", "")
            .replaceAll("captain-overlay-network", `$$$config_${env}`)
            .toLowerCase();
  ...
  if (serviceTemplate.volumes && serviceTemplate.volumes.length > 0) {
    for (const volume of serviceTemplate.volumes) {
      ...
      newService.volumes.push(
        `${source.replaceAll("$$cap_appname-", "$$$id-")}:${target}`
      );
    }
  }
